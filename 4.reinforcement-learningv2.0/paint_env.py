        ...
        for i in range(self.env_batch):
           # if self.env.imgid[i] <= 10:
            if self.env.imgid[i] == 10:
               ...
               ##DESCOMENTAR PARA PINTAREL GROUND TRUTH
               # gt = cv2.cvtColor((np.transpose(self.env.gt[i],(2, 1, 0))), cv2.COLOR_BGR2RGB)
               # cv2.imwrite('{}_gt_{}.png'.format(str(self.env.imgid[i]), str(step)), np.transpose(self.env.gt[i],(2, 1, 0)))
